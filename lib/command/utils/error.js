...
exports.format = function (command) {
  ...
  function textError (err) {
    return `Failed to ${command} the widget: ${err.toString()}`;
  }
...
function getValidationErrorReason (error) {
  ...
  if (property === 'name') {
    return 'Provide a valid widget name (1-255 characters).';
  } else if (error.expected) {
    return `The "${property}" widget property expects: ${error.expected}`;
  } else if (error.max) {
    return `The "${property}" widget property must have at most ${error.max} characters.`;
  } else {
